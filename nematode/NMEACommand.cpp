...
string NMEACommand::addChecksum(std::string s)
{
  ...
  ss << "$" << zz.str() << "*" << hex << uppercase << internal << setfill('0')
     << setw(2) << (int)checksum << "\r\n";
...
Table 2-4 Set Serial Port Data Format
 ...
 StopBits	1 			0,1		1. SiRF protocol is only valid for 8 data bits,
1stop bit, and no parity. Parity		0 			0=None, 1=Odd, 2=Even
 Checksum	*0C
...
std::string NMEACommandQueryRate::toString()
{
  ...
  ss << setfill('0') << setw(2) << messageID << "," << setfill('0') << setw(2)
     << mode << "," << setfill('0') << setw(2) << rate << "," << setfill('0')
     << setw(2) << checksumEnable;
  message = ss.str();
