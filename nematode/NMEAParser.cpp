...
std::string NMEAParseError::what() { return message; }
...
bool NMEASentence::valid() const { return isvalid; }
...
void NMEAParser::setSentenceHandler(
    std::string cmdKey, std::function<void(const NMEASentence&)> handler)
{
...
// calls the corresponding handler in eventTable, based on the 5 letter sentence
// code
void NMEAParser::readSentence(std::string cmd)
{
  ...
  {
    stringstream ss;
    ss << "New NMEA string was full of " << (beginsize - cmd.size())
       << " whitespaces!";
  ...
  // Call the "any sentence" event handler, even if invalid checksum, for
  // possible logging elsewhere.
  ...
  if (handler)
  {
    onInfo(nmea, string("Calling specific handler for sentence named \"") +
                     nmea.name + "\"");
    handler(nmea);
  }
  else
  {
    onWarning(nmea, string("Null event handler for type (name: \"") +
                        nmea.name + "\")");
  }
...
uint8_t NMEAParser::calculateChecksum(string s)
{
  ...
  //{
  //	ios_base::fmtflags oldflags = cout.flags();
  //	cout << "NMEA parser Info: calculated CHECKSUM for \""  << s << "\": 0x"
  //<< std::hex << (int)checksum << endl; 	cout.flags(oldflags);  //reset
  //}
  return checksum;
}
...
void NMEAParser::parseText(NMEASentence& nmea, string txt)
{
  ...
  {
    // No checksum is only a warning because some devices allow sending data
    // with no checksum.
  ...
  // above line parsing does not add a blank parameter if there is a comma at
  // the end...
  // so do it here.
  if (*(txt.end() - 1) == ',')
  {
  ...
  else
  {
    ...
    if (checki != string::npos)
    {
      ...
      {
        nmea.checksum = last.substr(
            checki + 1, last.size() - checki);  // extract checksum without '*'
        ...
        {
          onError(nmea, string("parseInt() error. Parsed checksum string was "
                               "not readable as hex. (\"") +
                            nmea.checksum + "\")");
        }

        onInfo(nmea, string("Checksum ok? ") +
                         (nmea.checksumOK() ? "YES" : "NO") + "!");
      }
    }
  }
  ...
  for (size_t i = 0; i < nmea.parameters.size(); i++)
  {
    if (!validParamChars(nmea.parameters[i]))
    {
      nmea.isvalid = false;
      stringstream ss;
      ss << "Invalid character (non-alpha-num) in parameter " << i
         << " (from 0): \"" << nmea.parameters[i] << "\"";
      onError(nmea, ss.str());
      break;
    }
  }
