...
// ------ Some helpers ----------
// Takes the NMEA lat/long format (dddmm.mmmm, [N/S,E/W]) and converts to
// degrees N,E only
...
double convertKnotsToKilometersPerHour(double knots) { return knots * 1.852; }
...
void GPSService::attachToParser(NMEAParser& _parser)
{
  ...
  $GPGSA		- gps receiver operating mode, satellites used in position and DOP
  values $GPGSV		- number of gps satellites in view, satellite ID,
  elevation,azimuth, and SNR $GPRMC		- time,date, position,course, and speed
  data $GPVTG		- course and speed information relative to the ground $GPZDA
  - 1pps timing message $PSRF150	- gps module "ok to send"
  */
  _parser.setSentenceHandler("PSRF150", [this](const NMEASentence& nmea) {
    this->read_PSRF150(nmea);
  });
  _parser.setSentenceHandler(
      "GPGGA", [this](const NMEASentence& nmea) { this->read_GPGGA(nmea); });
  _parser.setSentenceHandler(
      "GPGSA", [this](const NMEASentence& nmea) { this->read_GPGSA(nmea); });
  _parser.setSentenceHandler(
      "GPGSV", [this](const NMEASentence& nmea) { this->read_GPGSV(nmea); });
  _parser.setSentenceHandler(
      "GPRMC", [this](const NMEASentence& nmea) { this->read_GPRMC(nmea); });
  _parser.setSentenceHandler(
      "GPVTG", [this](const NMEASentence& nmea) { this->read_GPVTG(nmea); });
}
  ...
  try
  {
    ...
    if (this->fix.visibleSatellites < this->fix.trackingSatellites)
    {
      this->fix.visibleSatellites =
          this->fix
              .trackingSatellites;  // the visible count is in another sentence.
    }
...
void GPSService::read_GPGSV(const NMEASentence& nmea)
{
  ...
  */
  ...
  try
  {
    ...
    {
      this->fix.visibleSatellites =
          0;  // if no satellites are tracking, then none are visible!
    }  // Also NMEA defaults to 12 visible when chip powers on. Obviously not
       // right.
    ...
    int entriesInPage =
        (nmea.parameters.size() - 3) >> 2;  // first 3 are not satellite info
    ...
    // cout << "ALMANAC FINISHED page " << this->fix.almanac.processedPages << "
    // of " << this->fix.almanac.totalPages << endl;
    this->onUpdate();
  }
  catch (NumberConversionError& ex)
  {
...
void GPSService::read_GPRMC(const NMEASentence& nmea)
{
  ...
  */
  ...
  try
  {
    ...
    {
      lockupdate =
          this->fix.setlock(false);  // not A or V, so must be wrong... no lock
    }

    this->fix.speed = convertKnotsToKilometersPerHour(
        parseDouble(nmea.parameters[6]));  // received as knots, convert to km/h
