...
int main(int argc, char** argv)
{
  ...
  // parser.log = true;		// true: will spit out all sorts of parse info on
  // each sentence.
  ...
  gps.onUpdate += [&gps]() {
    cout << "\t\t\tPosition: " << gps.fix.latitude << "'N, "
         << gps.fix.longitude << "'E" << endl
         << endl;
  };

  // (optional) - Handle events when the parser receives each sentence
  parser.onSentence += [&gps](const NMEASentence& n) {
    cout << "Received " << (n.checksumOK() ? "good" : "bad")
         << " GPS Data: " << n.name << endl;
  };
  ...
  custom_parser.onSentence +=
      [](const NMEASentence& n) { cout << "Received $" << n.name << endl; };
  ...
  NMEACommandQueryRate cmd2;  // The $PSRF command that allows for GPS sentence
                              // selection and rate setting.
  NMEACommandQueryRate cmd3;  // The $PSRF command that allows for GPS sentence
                              // selection and rate setting.
  NMEACommandSerialConfiguration
      cmd4;  // The $PSRF command that can configure a UART baud rate.
  NMEAParser test_parser;
  test_parser.onSentence +=
      [&cmd1, &cmd2, &cmd3, &cmd4](const NMEASentence& n) {
  ...
  // Generate the NMEA sentence from the commands and send them back into the
  // test parser.
